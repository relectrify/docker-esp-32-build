...
RUN apt-get -qq update \
    && apt-get install -y git build-essential wget make libncurses-dev flex bison gperf python python-serial python-dev python-cryptography libssl-dev libffi-dev vim \
    && apt-get clean \
...
# Install ESP-IDF	
WORKDIR /esp	
RUN git clone --branch release/v3.1 --recurse-submodules https://github.com/espressif/esp-idf.git	
